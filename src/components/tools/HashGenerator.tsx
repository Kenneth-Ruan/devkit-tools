...
export default function HashGenerator() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([sha('SHA-1', input), sha('SHA-256', input), sha('SHA-512', input)]).then(([sha1, sha256, sha512]) => {
      if (!cancelled) setResults({ MD5: md5(input), 'SHA-1': sha1, 'SHA-256': sha256, 'SHA-512': sha512 });
    });
