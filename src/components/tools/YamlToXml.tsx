...
import { yamlToXml } from '@/lib/transforms';
...
export default function YamlToXml() {
  ...
  const { output, error } = useMemo(() => {
    ...
    try {
      return { output: yamlToXml(input, rootTag || 'root'), error: '' };
    } catch (e) {
  ...
  return (
    <div className="space-y-3">
      ...
      <SplitPane
        left={
          <div>
            <label className="text-xs text-slate-500 mb-1 block">YAML Input</label>
